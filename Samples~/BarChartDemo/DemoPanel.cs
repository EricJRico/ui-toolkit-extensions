...
namespace BarGraph.Demo
{
    ...
    abstract class DemoPanel
    {
        ...
        protected DemoPanel(string title)
        {
            Title = title;

            // Card container
            Card = new VisualElement();
            Card.style.backgroundColor     = new Color(0.06f, 0.06f, 0.08f, 1f);
            ...
            Card.style.borderBottomColor = Card.style.borderLeftColor = new Color(0.16f, 0.16f, 0.20f, 1f);
        ...
        /// <summary>Add panel-specific controls to <see cref="ControlsRow"/>.</summary>
        protected abstract void BuildControls();

        /// <summary>Regenerate data for the graph from current settings.</summary>
        public abstract void Regenerate();

        /// <summary>Called every frame by the orchestrator. Override for live panels.</summary>
        ...
        protected void AddOverlayToggle()
        {
            AddControl(MakeToggleButton("Overlay Off", "Overlay", false, on =>
            {
                HasOverlay = on;
                if (on)
                    ApplyOverlay();
